...
namespace rpos { namespace system { namespace util {
    ...
    */
    RPOS_CORE_API std::string to_uuid_string(const std::uint8_t* buffer, bool upperCase = false);
    ...
    * \brief Parse value from string
    */
    RPOS_CORE_API bool try_parse_uuid(const std::string& s, std::vector<std::uint8_t>& data);
    ...
    */
    ...
    {
        return wcs_to_acp(utf8_to_wcs(utf8));
    }

    inline std::string acp_to_utf8(const std::string& acp)
    {
        return wcs_to_utf8(acp_to_wcs(acp));
    }
    ...
    */
    ...
    static inline rtstring utf8_to_rt(const std::string& rcSrc) {
        return utf8_to_wcs(rcSrc);
    }

    static inline rtstring wcs_to_rt(const std::wstring& rcSrc) {
        return rcSrc;
    }

    static inline std::string rt_to_utf8(const rtstring& rcSrc) {
        return wcs_to_utf8(rcSrc);
    }

    static inline std::wstring rt_to_wcs(const rtstring& rcSrc) {
        return rcSrc;
    }
#else
    /**
    ...
    */
    ...
    static inline rtstring utf8_to_rt(const std::string& rcSrc) {
        return utf8_to_acp(rcSrc);
    }

    static inline rtstring wcs_to_rt(const std::wstring& rcSrc) {
        return wcs_to_acp(rcSrc);
    }

    static inline std::string rt_to_utf8(const rtstring& rcSrc) {
        return acp_to_utf8(rcSrc);
    }

    static inline std::wstring rt_to_wcs(const rtstring& rcSrc) {
        return acp_to_wcs(rcSrc);
    }
#endif
