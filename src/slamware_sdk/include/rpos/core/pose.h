...
namespace rpos {
    namespace core {
        ...
        public:
            ...
            explicit Quaternion(const Rotation&);
        public:
            Quaternion& operator=(const Quaternion &);
        public:
            rpos::core::Vector3f convertToEulerAngle() const;
        public:
            double x() const;
            double& x();
            double y() const;
        ...
        public:
            KeyFramePose();
            KeyFramePose(uint64_t id, const Pose& pose);
            KeyFramePose(uint64_t id, uint64_t timestamp, const Pose& pose);
            ~KeyFramePose(); 
        ...
        public:
            WeightedPose();
            WeightedPose(float weight,const Pose& pose);
            ~WeightedPose();
        ...
        struct RPOS_CORE_API TimedPose
        {
        public:
            TimedPose();
            TimedPose(const uint64_t& timestamp,const Pose& pose);
            ~TimedPose();

        public:
            bool operator==(const TimedPose& that) const;
            TimedPose& operator=(const TimedPose& that);
            bool operator<(const TimedPose& that) const;
