...
void Scan2MapLocation::InitParams()
{
    ...
    private_node_.param<std::string>("lidar_frame", lidar_frame_, "laser");
    ...
    private_node_.param<double>("ANGLE_UPPER_THRESHOLD", ANGLE_UPPER_THRESHOLD_, 30);    //最大变换角度
    ...
    private_node_.param<double>("Point_Quantity_THRESHOLD", Point_Quantity_THRESHOLD_, 100);   //点云数阈值,低于此值不匹配
...
bool Scan2MapLocation::ScanMatchWithICP(Eigen::Isometry3d &trans , PointCloudT::Ptr &cloud_scan_msg, PointCloudT::Ptr &cloud_map_msg)
{
    ...
    pcl::getTranslationAndEulerAngles(transfrom, x, y, z, roll, pitch, yaw);
     std::cout << "ICP transfrom: (" << x << ", " << y << ", " << (yaw*180)/3.1415 << ")" << std::endl;
